...
- `src/lib/server/bedrock.ts` appelle l’API InvokeModel (format natif Anthropic) de **Bedrock Runtime** et demande une sortie JSON structurée. Le profil est configurable ; le modèle doit accepter le format natif Anthropic, les images et les sorties structurées. Cette configuration est partagée avec CPS IA.
