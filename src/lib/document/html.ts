...
export function buildInvoiceHtml(invoice: Invoice, assets: DocumentAssets = browserAssets) {
  ...
  return `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Facture EXNOV ${e(v.numero)}</title><style>
  ...
  <div class="invoice-meta"><div class="invoice-number"><strong>FACTURE Nº ${e(v.numero)}</strong><br><strong>DATE</strong> ${e(v.date)}</div><div class="recipient"><p>POUR</p><p>${e(v.destinataire || "DESTINATAIRE")}</p>${v.reference ? `<p>REFERENCE: ${e(v.reference)}</p>` : ""}</div></div>
  ...
  <div class="closing"><p>Arrêté la présente facture à la somme de :</p><p>${e(v.montantEnLettres)}.</p><p class="signature">Signature</p><p class="thanks">Nous vous remercions de votre confiance</p></div>
