...
export async function parseInvoiceRequest(request: Request) {
  ...
  if (!reader) throw new RequestError("La facture est vide.", 400);
  ...
  try {
    while (true) {
      ...
      if (size > 500000) { await reader.cancel(); throw new RequestError("La facture est trop volumineuse.", 413); }
...
export function fileResponse(bytes: Uint8Array, numero: number, format: "pdf" | "docx") {
  return new Response(new Uint8Array(bytes), { headers: {
    "Content-Type": format === "pdf" ? "application/pdf" : "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
    "Content-Disposition": `attachment; filename="Facture-EXNOV-${numero}.${format}"`,
