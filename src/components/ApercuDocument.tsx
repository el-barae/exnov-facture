...
export function ApercuDocument({ html, label }: { html: string; label: string }) {
  ...
  return <section className={`preview-panel ${expanded ? "preview-expanded" : ""}`} aria-label={`Aperçu : ${label}`}>
    ...
    <div className="preview-scroll"><div ref={container} className="preview-canvas"><div className="paper-shadow" style={{ width: 794 * scale, height: height * scale }}><iframe ref={frame} title={`${label} au format A4`} srcDoc={html} sandbox="allow-scripts" style={{ width: 794, height, transform: `scale(${scale})`, transformOrigin: "top left" }} tabIndex={-1}/></div></div></div>
